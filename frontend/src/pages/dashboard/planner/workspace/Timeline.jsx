  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-extrabold text-ink">Timeline</h2>
          <p className="text-sm text-muted">Milestones, deadlines and the run-up to the event day.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <AskAiButton eventId={event.id} prompt={`What's overdue or coming up for ${event.title}?`} label="Ask AI" />
          <GenerateAiButton templateKey="planning_timeline" eventId={event.id} label="Timeline" />
          <GenerateAiButton templateKey="run_of_show" eventId={event.id} label="Run of show" />
          <Button size="sm" onClick={openCreate}><Icon name="Plus" className="size-4" /> Add milestone</Button>
        </div>
      </div>
      ...
      {milestones.length ? (
              ...
              <div className="rounded-card border border-line/80 bg-surface p-4 shadow-card">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => openEdit(m)} className="grid size-8 place-items-center rounded-btn text-muted hover:bg-canvas hover:text-ink">
                      <Icon name="PenLine" className="size-4" />
                    </button>
                    <button type="button" onClick={() => setRemoving(m)} className="grid size-8 place-items-center rounded-btn text-muted hover:bg-danger-soft hover:text-danger">
        ...
        <EmptyState icon="CalendarClock" title="No milestones yet" description="Map out the plan by adding your first milestone."
          action={<Button size="sm" onClick={openCreate}><Icon name="Plus" className="size-4" /> Add milestone</Button>} />
      )}
      ...
      <ConfirmDialog open={!!removing} onClose={() => setRemoving(null)} onConfirm={remove}
        title="Delete milestone?" confirmLabel="Delete" loading={busy} />
    </div>
  )
}
...
function MilestoneDrawer({ open, editing, eventId, onClose, onSaved }) {
  ...
  return (
    <Drawer open={open} onClose={onClose} title={editing ? 'Edit milestone' : 'Add milestone'}>
      <form onSubmit={submit} className="space-y-4">
        <Field label="Name" error={errors.name?.message} {...register('name', { required: 'A name is required' })} />
        <Textarea label="Description" rows={3} {...register('description')} />
        <SelectField label="Status" options={MILESTONE_STATUS_OPTIONS} {...register('status')} />
        <Field type="date" label="Due date" {...register('due_date')} />
        <div className="flex justify-end pt-2">
          <Button type="submit" loading={isSubmitting}>{editing ? 'Save' : 'Add milestone'}</Button>
        </div>
      </form>
    </Drawer>
  )
